...
Cannot assign to 'variable' because it is a 'read-only variable type'  
 ...
 This error occurs when an assignment to variable occurs in a read-only context. Read-only contexts include [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md) iteration variables, [using](../Topic/using%20Statement%20\(C%23%20Reference\).md) variables, and [fixed](../vs140/fixed-Statement--C#-Reference-.md) variables. To resolve this error, avoid assignments to a statement variable in `using` blocks, `foreach` statements, and `fixed` statements.  
  
## Example  
 The following example generates error CS1656 because it tries to replace complete elements of a collection inside a `foreach` loop. One way to work around the error is to change the `foreach` loop to a [for](../vs140/for--C#-Reference-.md) loop. Another way, not shown here, is to modify the members of the existing element; this is possible with classes, but not with structs.  
