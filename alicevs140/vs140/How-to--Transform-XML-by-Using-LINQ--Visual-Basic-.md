...
[XML Literals](../Topic/XML%20Literals%20\(Visual%20Basic\).md) make it easy to read XML from one source and transform it to a new XML format. You can take advantage of LINQ queries to retrieve the content to transform, or change content in an existing document to a new XML format.  
...
2.  Double-click the Module1.vb file created in the project to modify the Visual Basic code. Add the following code to the `Sub Main` of the `Module1` module. This code creates the source XML document as an <xref:System.Xml.Linq.XDocument?qualifyHint=False> object.  
    ...
    ```  
     ...
     [Applications typically load XML from an external source such as a string passed as a parameter, or a supplied file or stream. For a demonstration of how to load XML from an external source, see How to: Load XML from a File, String, or Stream (Visual Basic)](../Topic/How%20to:%20Load%20XML%20from%20a%20File,%20String,%20or%20Stream%20\(Visual%20Basic\).md).  
...
## See Also  
 ...
 [How to: Load XML from Files (Visual Basic)](../Topic/How%20to:%20Load%20XML%20from%20a%20File,%20String,%20or%20Stream%20\(Visual%20Basic\).md)   
