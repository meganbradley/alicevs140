...
## Return Value  
 ...
 If an invalid argument is passed to the function, the invalid parameter handler is invoked, as described in [Parameter Validation](../vs140/Parameter-Validation.md). If execution is allowed to continue, `errno` is set to `EINVAL` and `EINVAL` is returned.  
...
>  The `_O_TRUNC` flag destroys the contents of the specified file.  
 ...
 `_sopen_s` applies the current file-permission mask to `pmode` before the permissions are set. (See [_umask](../vs140/_umask.md).)  
...
|`_wsopen_s`|<io.h> or <wchar.h>|<fcntl.h>, <sys/types.h>, <sys/stat.h>, <share.h>|  
 ...
 `_sopen_s` and `_wsopen_s` are Microsoft extensions. For more compatibility information, see [Compatibility](../vs140/Compatibility.md).  
