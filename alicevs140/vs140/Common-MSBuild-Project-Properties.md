...
The following table lists frequently used properties that are defined in the Visual Studio project files or included in .targets files that MSBuild provides.  
 ...
 Project files in Visual Studio (.csproj,.vbproj, vcxproj, and others) contain MSBuild XML code that runs when you build a project by using the IDE. Projects typically import one or more .targets files to define their build process. For more information, see [MSBuild .Targets Files](../vs140/MSBuild-.Targets-Files.md).  
...
|ApplicationManifest|Specifies the path of the file that is used to generate external User Account Control (UAC) manifest information. Applies only to Visual Studio projects targeting [!INCLUDE[windowsver](../vs140/includes/windowsver_md.md)].<br /><br /> In most cases, the manifest is embedded. However, if you use Registration Free COM or [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] deployment, then the manifest can be an external file that is installed together with your application assemblies. For more information, see the NoWin32Manifest property in this topic.|  
|AssemblyOriginatorKeyFile|Specifies the file that's used to sign the assembly (.snk or .pfx) and that's passed to the [ResolveKeySource Task](../vs140/ResolveKeySource-Task.md) to generate the actual key that's used to sign the assembly.|  
...
|BaseOutputPath|Specifies the base path for the output file. If it is set, [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] will use `OutputPath = $(BaseOutputPath)\$(Configuration)\`. Example syntax: `<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>`|  
...
|BuildInParallel|A boolean value that indicates whether project references are built or cleaned in parallel when Multi-Proc [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] is used. The default value is `true`, which means that projects will be built in parallel if the system has multiple cores or processors.|  
|BuildProjectReferences|A boolean value that indicates whether project references are built by [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)]. Set `false` if you are building your project in the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] integrated development environment (IDE), `true` if otherwise.|  
...
|CscToolPath|The path of csc.exe, the [!INCLUDE[csprcs](../vs140/includes/csprcs_md.md)] compiler.|  
...
|DisableFastUpToDateCheck|A boolean value that applies to [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] only. The [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] build manager uses a process called FastUpToDateCheck to determine whether a project must be rebuilt to be up to date. This process is faster than using [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] to determine this. Setting the DisableFastUpToDateCheck property to `true` lets you bypass the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] build manager and force it to use [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] to determine whether the project is up to date.|  
...
|ExcludeDeploymentUrl|The [GenerateDeploymentManifest Task](../Topic/GenerateDeploymentManifest%20Task.md) adds a  deploymentProvider tag to the deployment manifest if the project file includes any of the following elements:<br /><br /> -   UpdateUrl<br />-   InstallUrl<br />-   PublishUrl<br /><br /> Using ExcludeDeploymentUrl, however, you can prevent the deploymentProvider tag from being added to the deployment manifest even if any of the above URLs are specified. To do this, add the following property to your project file:<br /><br /> `<ExcludeDeploymentUrl>true</ExcludeDeploymentUrl>` **Note:**  ExcludeDeploymentUrl is not exposed in the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] IDE and can be set only by manually editing the project file. Setting this property does not affect publishing within [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]; that is, the deploymentProvider tag will still be added to the URL specified by PublishUrl.|  
...
|NoVBRuntimeReference|A boolean value that indicates whether the [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] runtime (Microsoft.VisualBasic.dll) should be included as a reference in the project.|  
|NoWin32Manifest|A boolean value that indicates whether User Account Control (UAC) manifest information will be embedded in the application's executable. Applies only to Visual Studio projects targeting [!INCLUDE[windowsver](../vs140/includes/windowsver_md.md)]. In projects deployed using [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] and Registration-Free COM, this element is ignored. `False` (the default value) specifies that User Account Control (UAC) manifest information be embedded in the application's executable. `True` specifies that UAC manifest information not be embedded.<br /><br /> This property applies only to [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] projects targeting [!INCLUDE[windowsver](../vs140/includes/windowsver_md.md)]. In projects deployed using [!INCLUDE[ndptecclick](../vs140/includes/ndptecclick_md.md)] and Registration-Free COM, this property is ignored.<br /><br /> You should add NoWin32Manifest only if you do not want [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] to embed any manifest information in the application's executable; this process is called *virtualization*. To use virtualization, set `<ApplicationManifest>` in conjunction with `<NoWin32Manifest>` as follows:<br /><br /> -   For [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] projects, remove the `<ApplicationManifest>` node. (In [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] projects, `<NoWin32Manifest>` is ignored when an `<ApplicationManifest>` node exists.)<br />-   For [!INCLUDE[csprcs](../vs140/includes/csprcs_md.md)] projects, set `<ApplicationManifest>` to `False` and `<NoWin32Manifest>` to `True`. (In [!INCLUDE[csprcs](../vs140/includes/csprcs_md.md)] projects, `<ApplicationManifest>` overrides `<NoWin32Manifest>`.)|  
...
|TargetFrameworkVersion|The version of the [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] that is required to run the application that you are building. Specifying this lets you reference certain framework assemblies that you may not be able to reference otherwise.|  
...
|UseHostCompilerIfAvailable|A boolean parameter that, if `true`, causes the build task to use the in-process compiler object, if it is available. This parameter is used only by [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].|  
...
## See Also  
 [Common MSBuild Items](../vs140/Common-MSBuild-Project-Items.md)