...
## Configurations and Platforms  
 Configurations are represented in [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] projects by properties grouped in a `PropertyGroup` element that contains a `Condition` attribute. [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] looks at these conditions in order to create a list of project configurations and platforms to display. To successfully extract this list, the conditions must have a format similar to the following:  
...
```  
 ...
 [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] looks at the conditions on `PropertyGroup`, `ItemGroup`, `Import`, property, and item elements for this purpose.  
...
-   In a target of the project, there must be a task named `Vbc` for [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] projects.  
...
## Building Solutions  
 ...
 When building inside [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], the property `$(BuildingInsideVisualStudio)` is set to `true`. This can be used in your project or .targets files to cause the build to behave differently.  
...
## Debugging  
 In order to find and launch the output assembly and attach the debugger, [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] needs the properties `OutputPath`, `AssemblyName`, and `OutputType` correctly defined. The debugger will fail to attach if the build process did not cause the compiler to generate a .pdb file.  
...
-   Assembly references:  
    ...
    -   `ResolvedFrom`, set to "{TargetFrameworkDirectory}" if it was resolved from the [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] directory.  
