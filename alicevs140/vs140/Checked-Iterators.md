...
> [!IMPORTANT]
>  Use `_ITERATOR_DEBUG_LEVEL` to control [_SECURE_SCL](../vs140/_SECURE_SCL.md). For more information, see [_ITERATOR_DEBUG_LEVEL](../vs140/_ITERATOR_DEBUG_LEVEL.md).  
...
-   If an output iterator is an unchecked iterator you will get unchecked behavior on calls to the standard function (for example, `std::copy`).  
 ...
 A checked iterator refers to an iterator that will call `invalid_parameter_handler` if you attempt to move past the boundaries of the container. For more information about `invalid_parameter_handler`, see [Parameter Validation](../vs140/Parameter-Validation.md).  
