...
2.  Let’s add a message that lets us know that the VSPackage has loaded. We use the VSPackage’s Initialize() method to do this, because you can get Visual Studio services only after the VSPackage has been sited. (For more information about getting services, see [How to: Get a Service](../Topic/How%20to:%20Get%20a%20Service.md).) Replace the Initialize() method of FirstPackage with code that gets the <xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell?qualifyHint=False> service, gets the <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIShell?qualifyHint=False> interface, and calls its <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIShell.ShowMessageBox?qualifyHint=False> method.  
