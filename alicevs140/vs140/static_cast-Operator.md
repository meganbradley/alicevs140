...
```  
 ...
 The `static_cast` operator cannot cast away the `const`, `volatile`, or `__unaligned` attributes. See [const_cast Operator](../vs140/const_cast-Operator.md) for information on removing these attributes.  
  
 Due to the danger of performing unchecked casts on top of a relocating garbage collector, the use of `static_cast` should only be in performance-critical code when you are certain it will work correctly. If you must use `static_cast` in release mode, substitute it with [safe_cast](../vs140/safe_cast--C---Component-Extensions-.md) in your debug builds to ensure success.  
