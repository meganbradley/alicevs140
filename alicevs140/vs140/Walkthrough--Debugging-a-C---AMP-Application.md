...
-   Make sure that line numbers are displayed in the text editor. For more information, see [How to: Display Line Numbers in the Editor](../vs140/How-to--Display-Line-Numbers-in-the-Editor.md).  
  
-   Make sure you are running [!INCLUDE[win8](../vs140/includes/win8_md.md)] or [!INCLUDE[winserver8](../vs140/includes/winserver8_md.md)] to support debugging on the software emulator.  
...
4.  Set breakpoints on the lines of code shown in the following illustration (approximately lines 67 line 70).  
     ...
     ![CPU breakpoints](../vs140/media/CampCpuBreakpoints.png "CampCpuBreakpoints")  
CPU breakpoints  
...
6.  Set a breakpoint at line 30, as shown in the following illustration.  
     ...
     ![GPU breakpoints](../vs140/media/CampGpuBreakpoints.png "CampGpuBreakpoints")  
GPU breakpoint  
...
2.  Dock the GPU Threads window at the bottom of Visual Studio. Choose the **Expand Thread Switch** button to display the tile and thread text boxes. The GPU Threads window shows the total number of active and blocked GPU threads, as shown in the following illustration.  
     ...
     ![GPU Threads window with 4 active threads](../vs140/media/CampC.PNG "CampC")  
GPU Threads window  
...
3.  Make sure that **Threads** is selected in the list in the upper-left corner. In the following illustration, the Parallel Stacks window shows a call-stack focused view of the GPU threads that you saw in the GPU Threads window.  
     ...
     ![Parallel Stacks window with 4 active threads](../vs140/media/CampD.png "CampD")  
Parallel Stacks window  
  
     32 threads went from `_kernel_stub` to the lambda statement in the `parallel_for_each` function call and then to the `sum_kernel_tiled` function, where the parallel reduction occurs. 28 out of the 32 threads have progressed to the [tile_barrier::wait](../vs140/tile_barrier--wait-Method.md) statement and remain blocked at line 22, whereas the other 4 threads remain active in the `sum_kernel_tiled` function at line 30.  
     ...
     ![DataTip for Parallel Stacks window](../vs140/media/CampE.png "CampE")  
GPU thread DataTip  
...
5.  Select the **Add Watch** column header again, type `localA[localIdx[0]]`, and then choose the Enter key.  
     ...
     ![Parallel Watch window with sorted results](../vs140/media/CampF.PNG "CampF")  
 Results of sort  
...
3.  Choose the flag symbol on the left side of the row that contains the four threads that are now active.  
     ...
     ![GPU Threads window with flagged threads](../vs140/media/CampG.PNG "CampG")  
...
4.  If you want to focus on the four threads that you flagged, you can choose to show, in the GPU Threads, Parallel Watch, and Parallel Stacks windows, only the flagged threads.  
     ...
     ![Debug Location toolbar with Show Only Flagged icon](../vs140/media/CampH.png "CampH")  
...
## Freezing and Thawing GPU Threads  
 You can freeze (suspend) and thaw (resume) GPU threads from either the GPU Threads window or the Parallel Watch window. You can freeze and thaw CPU threads the same way; for information, see [How to: Use the Threads Window](../vs140/How-to--Use-the-Threads-Window.md).  
...
3.  Open the shortcut menu for the active row and then choose **Freeze**.  
     ...
     ![GPU Threads windows showing frozen threads](../vs140/media/CampK.PNG "CampK")  
...
1.  On the shortcut menu for one of the threads in the **GPU Threads** window, choose **Group By**, **Address**.  
     ...
     The threads in the GPU Threads window are grouped by address. The address corresponds to the instruction in disassembly where each group of threads is located. 24 threads are at line 22 where the [tile_barrier::wait Method](../vs140/tile_barrier--wait-Method.md) is executed. 12 threads are at the instruction for the barrier at line 32. Four of these threads are flagged. Eight threads are at the breakpoint at line 30. Four of these threads are frozen. The following illustration shows the grouped threads in the GPU Threads window.  
  
     ![GPU Threads window with threads grouped by Address](../vs140/media/CampL.PNG "CampL")  
