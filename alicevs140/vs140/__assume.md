...
>  A program must not contain an invalid `__assume` statement on a reachable path. If the compiler can reach an invalid `__assume` statement, the program might cause unpredictable and potentially dangerous behavior.  
 ...
 Use `__assume` in an [ASSERT](../vs140/_ASSERT--_ASSERTE--_ASSERT_EXPR-Macros.md) only when the assert is not recoverable. Do not use `__assume` in an assert for which you have subsequent error recovery code because the compiler might optimize away the error-handling code.  
...
|---------------|------------------|  
|`__assume`|x86, ARM, [!INCLUDE[vcprx64](../vs140/includes/vcprx64_md.md)]|  
