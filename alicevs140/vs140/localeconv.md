...
## Return Value  
 `localeconv` returns a pointer to a filled-in object of type [struct lconv](../vs140/Standard-Types.md). The values contained in the object can be overwritten by subsequent calls to `localeconv` and do not directly modify the object. Calls to [setlocale](../vs140/setlocale--_wsetlocale.md) with `category` values of `LC_ALL`, `LC_MONETARY`, or `LC_NUMERIC` overwrite the contents of the structure.  
