...
|[subtract_with_carry_engine](../vs140/subtract_with_carry_engine-Class.md)|Generates a random sequence by using the subtract-with-carry algorithm. An improvement on `linear_congruential_engine`, but much lower quality and performance than `mersenne_twister_engine`.|  
...
|||  
|-|-|  
|[bernoulli_distribution](../vs140/bernoulli_distribution-Class.md)|Produces a Bernoulli distribution of `bool` values.|  
...
-   `template <class Seq> void seed(Seq& q)` seeds the generator by using a [seed_seq](../vs140/seed_seq-Class.md)`Seq`.  
