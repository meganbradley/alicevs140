...
This topic describes how to add a toolbar button that contains a Windows control to a toolbar. In MFC, a toolbar button must be a [CMFCToolBarButton Class](../vs140/CMFCToolBarButton-Class.md)-derived class, for example [CMFCToolBarComboBoxButton](../vs140/CMFCToolBarComboBoxButton-Class.md), [CMFCToolBarEditBoxButton](../vs140/CMFCToolBarEditBoxButton-Class.md), [CMFCDropDownToolbarButton](../vs140/CMFCDropDownToolbarButton-Class.md), or [CMFCToolBarMenuButton](../vs140/CMFCToolBarMenuButton-Class.md).  
  
## Adding Controls to Toolbars  
 To add a control to a toolbar, follow these steps:  
  
1.  Reserve a dummy resource ID for the button in the parent toolbar resource. For more information about how to create buttons by using the Toolbar Editor in Visual Studio, see the [Toolbar Editor](../vs140/Toolbar-Editor.md) topic.  
...
3.  In the message handler that processes the `AFX_WM_RESETTOOLBAR` message, do the following:  
    ...
    2.  Replace the dummy button with the new control by using [CMFCToolBar::ReplaceButton](../vs140/CMFCToolBar--ReplaceButton.md). You can construct the button object on the stack, because `ReplaceButton` copies the button object and maintains the copy.  
...
## Toolbar Controls and Customization  
 ...
 When you enable customization, you create the **Customize** dialog box in the customization handler `OnViewCustomize` by using the [CMFCToolbarsCustomizeDialog](../vs140/CMFCToolBarsCustomizeDialog-Class.md) class. Before you display the **Customize** dialog box by calling [CMFCToolbarsCustomizeDialog::Create](../vs140/CMFCToolBarsCustomizeDialog--Create.md), call [CMFCToolbarsCustomizeDialog::ReplaceButton](../vs140/CMFCToolBarsCustomizeDialog--ReplaceButton.md) to replace the standard button with the new control.  
  
## Example: Creating a Find Combo Box  
 This section describes how to create a `Find` combo box control that appears on a toolbar and contains recently-used search strings. The user can type a string in the control and then press the enter key to search a document, or press the escape key to return the focus to the main frame. This example assumes that the document is displayed in a [CEditView](../vs140/CEditView-Class.md)-derived view.  
...
2.  Create a new class, `CFindComboBox`, derived from [CComboBox](../vs140/CComboBox-Class.md).  
...
4.  Create a class for the `Find` combo box button, derived from [CMFCToolbarComboBoxButton](../vs140/CMFCToolBarComboBoxButton-Class.md). In this example, it is named `CFindComboButton`.  
...
5.  The constructor of `CMFCToolbarComboBoxButton` takes three parameters: the command ID of the button, the button image index, and the style of the combo box. Set these parameters as follows:  
    ...
    3.  For a list of available combo box styles, see [Combo-Box Styles](../vs140/Combo-Box-Styles.md).  
...
7.  Use the [IMPLEMENT_SERIAL](../vs140/IMPLEMENT_SERIAL.md) macro to make the combo button persistent. The workspace manager automatically loads and saves the button's state in the Windows registry.  
  
8.  Implement the `ID_EDIT_FIND_COMBO` handler in your document view. Use [CMFCToolBar::GetCommandButtons](../vs140/CMFCToolBar--GetCommandButtons.md) with `ID_EDIT_FIND_COMBO` to retrieve all `Find` combo box buttons. There can be several copies of a button with the same command ID because of customization.  
  
9. In the ID_EDIT_FIND message handler `OnFind`, use [CMFCToolBar::IsLastCommandFromButton](../vs140/CMFCToolBar--IsLastCommandFromButton.md) to determine whether the find command was sent from the `Find` combo box button. If so, find the text and add the search string to the combo box.  
...
2.  In the `AFX_WM_RESETTOOLBAR` handler, examine the toolbar ID, that is, the `WPARAM` of the `AFX_WM_RESETTOOLBAR` message. If the toolbar ID is equal to that of the toolbar that contains the `Find` combo box button, call [CMFCToolBar::ReplaceButton](../vs140/CMFCToolBar--ReplaceButton.md) to replace the `Find` button (that is, the button with the command ID `ID_EDIT_FIND)` with a `CFindComboButton` object.  
...
### Adding the Find Control to the Customize Dialog Box  
 In the customization handler `OnViewCustomize`, call [CMFCToolbarsCustomizeDialog::ReplaceButton](../vs140/CMFCToolBarsCustomizeDialog--ReplaceButton.md) to replace the `Find` button (that is, the button with the command ID `ID_EDIT_FIND)` with a `CFindComboButton` object.  
