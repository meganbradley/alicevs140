...
>  An instance of `IDiagram` (and its subtypes such as `IClassDiagram`) is valid only within the command you are processing. It is not recommended to keep an `IDiagram` object in a variable that persists while control is returned to the user.  
 ...
 For more information, see [How to Add Commands and Gestures to a UML Diagram](../Topic/Define%20a%20menu%20command%20on%20a%20modeling%20diagram.md).  
...
```  
 ...
 Instances of `IDiagram` and its subtypes are not valid after you return control to [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
...
##  <a name="AlignCommand"></a> Example: Command for Aligning Shapes  
 ...
 To make the command available, add this code to a menu command project, and then deploy the resulting extension to your users. For more information, see [How to Define a Menu Command](../Topic/Define%20a%20menu%20command%20on%20a%20modeling%20diagram.md).  
