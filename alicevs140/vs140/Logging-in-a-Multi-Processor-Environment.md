...
### Central Logging Model  
 ...
 Although only <xref:Microsoft.Build.Framework.ILogger?qualifyHint=False> is required to be implemented by the central logger, we recommend that you also implement <xref:Microsoft.Build.Framework.INodeLogger?qualifyHint=False> if you want the central logger to initialize with the number of nodes that are participating in the build. The following overload of the <xref:Microsoft.Build.Framework.ILogger.Initialize?qualifyHint=False> method is invoked when the engine initializes the logger:  
...
### Distributed Logging Model  
 ...
 You can create a forwarding logger by implementing the <xref:Microsoft.Build.Framework.IForwardingLogger?qualifyHint=False> interface, which derives from <xref:Microsoft.Build.Framework.ILogger?qualifyHint=False>. The interface is defined as:  
...
```  
 ...
 To forward events in a forwarding logger, call the <xref:Microsoft.Build.Framework.IEventRedirector.ForwardEvent?qualifyHint=False> method of the <xref:Microsoft.Build.Framework.IEventRedirector?qualifyHint=False> interface. Pass the appropriate <xref:Microsoft.Build.Framework.BuildEventArgs?qualifyHint=False>, or a derivative, as the parameter.  
