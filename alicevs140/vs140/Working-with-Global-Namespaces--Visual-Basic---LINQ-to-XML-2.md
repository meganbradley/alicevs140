...
One of the key features of XML literals in Visual Basic is the capability to declare XML namespaces by using the `Imports` statement. Using this feature, you can declare an XML namespace that uses a prefix, or you can declare a default XML namespace.  
 ...
 You can get an <xref:System.Xml.Linq.XNamespace?qualifyHint=False> object that corresponds to a global namespace using the `GetXmlNamespace` method.  
  
## Examples of Global Namespaces  
 The following example declares a default global namespace by using the `Imports` statement, and then uses an XML literal to initialize an <xref:System.Xml.Linq.XElement?qualifyHint=False> object in that namespace:  
...
## XNamespace and Global Namespaces  
 You can get an <xref:System.Xml.Linq.XNamespace?qualifyHint=False> object by using the `GetXmlNamespace` method:  
