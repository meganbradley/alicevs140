...
### Remarks  
 The specified format is one previously placed in the control object using the [DelayRenderData](../Topic/COleDataSource%20Class.md#coledatasource__delayrenderdata) member function for delayed rendering. The default implementation of this function simply returns **FALSE**.  
...
### Return Value  
 A pointer to the previously selected `CFont` object. You should use [CDC::SelectObject](../vs140/CDC-Class.md#cdc__selectobject) to select this font back into the device context when you are finished.  
...
```  
void ThrowError(    SCODE sc ,    UINT nDescriptionID ,    UINT nHelpID = -1 ); void ThrowError(    SCODE sc ,    LPCTSTR pszDescription = NULL,    UINT nHelpID = 0 );  
