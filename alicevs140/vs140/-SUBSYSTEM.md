...
-   The values of `major` and `minor` must be from 0 through 65,535.  
 ...
 The choice of subsystem affects the default starting address for the program. For more information, see [/ENTRY (Entry-Point Symbol)](../vs140/-ENTRY--Entry-Point-Symbol-.md), the linker /ENTRY:*function* option.  
  
 For more information, including the minimum and default values for the major and minor version numbers for each subsystem, see the [/SUBSYSTEM](../Topic/-SUBSYSTEM%20\(Specify%20Subsystem\).md) linker option.  
