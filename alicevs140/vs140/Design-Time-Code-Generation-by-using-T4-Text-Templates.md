...
3.  Open the file. It will already contain the following directives:  
    ...
    ```  
     ...
     If you added the template to a [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] project, the language attribute will be "`VB`".  
...
2.  Save the .tt file, and inspect the generated .txt file again. It lists the squares of the numbers from 0 to 10.  
 ...
 Notice that statements are enclosed within `<#...#>`, and single expressions within `<#=...#>`. For more information, see [Writing a Text Template](../vs140/Writing-a-T4-Text-Template.md).  
  
 If you write the generating code in [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)], the `template` directive should contain `language="VB"`. `"C#"` is the default.  
...
### Getting data from [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]  
 To use services provided in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], set the `hostSpecific` attribute and load the `EnvDTE` assembly. You can then use IServiceProvider.GetCOMService() to access DTE and other services. For example:  
...
```  
 ...
 For more information, see [Code Generation in a Build Process](../Topic/Code%20Generation%20in%20a%20Build%20Process.md).  
...
1.  To your [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] project, add a file of the type that you want to generate, such as a `.cs`, `.vb`, or `.resx` file.  
...
5.  Insert the following lines at the beginning of the file:  
    ...
    ```  
     ...
     If you want to write the generating code of your template in [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)], set the `language` attribute to `"VB"` instead of `"C#"`.  
...
## Guidelines for Code Generation  
 Please see [Guidelines for Writing Text Templates](../Topic/Guidelines%20for%20Writing%20T4%20Text%20Templates.md).  
...
## See Also  
 [Guidelines for Writing Text Templates](../Topic/Guidelines%20for%20Writing%20T4%20Text%20Templates.md)