...
1.  The accessor needs to contain an object of type [CBookmark](../vs140/CBookmark-Class.md). The `nSize` parameter specifies the size of the bookmark buffer in bytes (typically 4 for 32-bit platforms and 8 for 64-bit platforms). Add the following declaration to the column data members in the user record class:  
