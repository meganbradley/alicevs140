...
### Remarks  
 `crbegin` is used with a reversed list just as [begin](#list__begin) is used with a `list`.  
...
### Remarks  
 `crend` is used with a reversed list just as [end](#list__end) is used with a `list`.  
...
### Remarks  
 ...
 If the ranged splice inserts `N` elements, and `&Source != this`, an object of class [iterator](../vs140/forward_list-Class.md#forward_list__iterator) is incremented `N` times.  
