...
>  This topic applies to objects derived from `CRecordset` in which bulk row fetching has not been implemented. If you are using bulk row fetching, see [Recordset: Fetching Records in Bulk (ODBC)](../vs140/Recordset--Fetching-Records-in-Bulk--ODBC-.md).  
 ...
 As a supplement, you might want to read [Record Field Exchange: How RFX Works](../vs140/Record-Field-Exchange--How-RFX-Works.md), which describes the corresponding role of RFX in update operations.  
  
##  <a name="_core_adding_a_record"></a> Adding a Record  
 Adding a new record to a recordset involves calling the recordset's [AddNew](../vs140/CRecordset--AddNew.md) member function, setting the values of the new record's field data members, and calling the [Update](../vs140/CRecordset--Update.md) member function to write the record to the data source.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
    ...
    2.  If there are changes, **Update** constructs a SQL **INSERT** statement. The columns represented by all dirty field data members are listed in the **INSERT** statement. To force a column to be included, call the [SetFieldDirty](../vs140/CRecordset--SetFieldDirty.md) member function:  
...
##  <a name="_core_editing_an_existing_record"></a> Editing an Existing Record  
 Editing an existing record in a recordset involves scrolling to the record, calling the recordset's [Edit](../vs140/CRecordset--Edit.md) member function, setting the values of the new record's field data members, and calling the [Update](../vs140/CRecordset--Update.md) member function to write the changed record to the data source.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
    ...
    3.  **Update** commits the changes — executes the **UPDATE** statement — and the record is changed on the data source, but not committed if a transaction is in progress (see [Transaction: Performing a Transaction in a Recordset (ODBC)](../vs140/Transaction--Performing-a-Transaction-in-a-Recordset--ODBC-.md) for information about how the transaction affects the update). ODBC keeps a copy of the record, which also changes.  
...
##  <a name="_core_deleting_a_record"></a> Deleting a Record  
 Deleting a record from a recordset involves scrolling to the record and calling the recordset's [Delete](../vs140/CRecordset--Delete.md) member function. Unlike `AddNew` and **Edit**, **Delete** does not require a matching call to **Update**.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
 ...
 For information about the SQL statements used in update operations, see [SQL](../vs140/SQL.md).  
  
## See Also  
 [Recordset (ODBC)](../vs140/Recordset--ODBC-.md)   
 [Recordset: More About Updates (ODBC)](../vs140/Recordset--More-About-Updates--ODBC-.md)   
