 ...
 **Various messages in MFC or ATL code**  
 ...
 `C4996` can also occur if you use MFC or ATL functions that were deprecated for security reasons. To suppress these warnings, see [_AFX_SECURE_NO_WARNINGS](../vs140/_AFX_SECURE_NO_WARNINGS.md) and [_ATL_SECURE_NO_WARNINGS](../vs140/_ATL_SECURE_NO_WARNINGS.md).  
...
## Example  
 C4996 can also occur if you do not use a checked iterator when compiling with `_ITERATOR_DEBUG_LEVEL` defined (set to 1 by default for debug mode builds).  See [Checked Iterators](../vs140/Checked-Iterators.md) for more information.  
