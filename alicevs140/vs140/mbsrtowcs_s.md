...
|The destination buffer is too small to contain the converted string (unless `count` is `_TRUNCATE`; for more information, see Remarks)|`ERANGE`|  
 ...
 If any one of these conditions occurs, the invalid parameter exception is invoked as described in [Parameter Validation](../vs140/Parameter-Validation.md) . If execution is allowed to continue, the function returns an error code and sets `errno` as indicated in the table.  
...
-   The number of wide characters stored in the `wcstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../vs140/_TRUNCATE.md), `mbsrtowcs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
