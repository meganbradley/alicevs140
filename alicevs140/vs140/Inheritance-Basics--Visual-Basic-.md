...
## The MyClass Keyword  
 The `MyClass` keyword behaves like an object variable that refers to the current instance of a class as originally implemented. `MyClass` resembles `Me`, but every method and property call on `MyClass` is treated as if the method or property were [NotOverridable](../vs140/NotOverridable--Visual-Basic-.md). Therefore, the method or property is not affected by overriding in a derived class.  
