...
You can profile on compute nodes of Microsoft Windows HPC clusters by using the sampling method of the [!INCLUDE[vsPreExt](../vs140/includes/vsPreExt_md.md)] or [!INCLUDE[vsUltExt](../vs140/includes/vsUltExt_md.md)] Profiling Tools. For more information about HPC see [Windows HPC](http://go.microsoft.com/fwlink/?LinkId=165393) on the Microsoft Web site.  
  
## Prerequisites  
 To profile on a HPC compute node, you must do the following:  
  
-   Install Microsoft HPC Pack 2008 on the same computer as [!INCLUDE[vsPreLong](../vs140/includes/vsPreLong_md.md)]. The computer does not have to be part of the HPC cluster. You can install the HPC Pack at the [Microsoft Download Center](http://go.microsoft.com/fwlink/?LinkID=177414).  
  
-   Install the [!INCLUDE[net_v40_long](../vs140/includes/net_v40_long_md.md)] and the stand-alone version of the Profiling Tools on the HPC compute node. Install programs for both the [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] and the stand alone profiler are available on the [!INCLUDE[vsPreShort](../vs140/includes/vsPreShort_md.md)] installation media. **Note** You must restart the compute after you have installed [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] and before you install the Profiling Tools.  
  
 To install the [!INCLUDE[net_v40_long](../vs140/includes/net_v40_long_md.md)] and the stand-alone Profiling Tools on an active HPC compute node and enable profiling on the cluster machine, follow these steps:  
...
|*%FxPath%*|Path to the [!INCLUDE[net_v40_long](../vs140/includes/net_v40_long_md.md)] installer. On the [!INCLUDE[vsPreShort](../vs140/includes/vsPreShort_md.md)] installation media the path is: WCU\dotNetFramework\dotNetFx40_Full_x86_x64.exe|  
|*%ProfilerPath%*|Path to the standalone version of the Profiling Tools installer. On the [!INCLUDE[vsPreShort](../vs140/includes/vsPreShort_md.md)] installation media the path is: Standalone Profiler\x64\vs_profiler.exe|  
...
3.  On the second page of the wizard, select the application that you want to profile.  
    ...
    -   To profile a project that is currently open in [!INCLUDE[vs_current_short](../vs140/includes/vs_current_short_md.md)], select the **One or more available projects** option and then select the project name from the list.  
...
|Property|Description|  
|--------------|-----------------|  
|**Project name**|The name of the current [!INCLUDE[vs_current_short](../vs140/includes/vs_current_short_md.md)] project or solution.|  
