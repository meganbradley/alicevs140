...
## Cause  
 A transparent method requires a <xref:System.Security.Permissions.SecurityAction.LinkDemand?qualifyHint=False> or other security demand.  
...
## How to Fix Violations  
 To fix a violation of this rule, remove the link demand on the transparent method or mark the method with <xref:System.Security.SecuritySafeCriticalAttribute?qualifyHint=False> attribute if it is performing security checks, such as security demands.  
  
## When to Suppress Warnings  
 Do not suppress a warning from this rule.  
  
## Example  
 In the following example, the rule fires on the method because the method is transparent and is marked with a LinkDemand <xref:System.Security.PermissionSet?qualifyHint=False> that contains an <xref:System.Security.Permissions.SecurityAction.LinkDemand?qualifyHint=False>.  
