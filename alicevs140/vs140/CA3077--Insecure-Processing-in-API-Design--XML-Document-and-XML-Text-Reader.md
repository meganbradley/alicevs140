...
## Cause  
 When designing an API derived from XMLDocument and XMLTextReader, be mindful of <xref:System.Xml.XmlReaderSettings.DtdProcessing?qualifyHint=False>.  Using insecure DTDProcessing instances when referencing or resolving external entity sources or setting insecure values in the XML may lead to information disclosure.  
...
-   <xref:System.Xml.XmlDocument?qualifyHint=False> or <xref:System.Xml.XmlTextReader?qualifyHint=False> classes use default resolver values for DTD processing    .  
  
-   No constructor is defined for the XmlDocument or XmlTextReader derived classes or no secure value is used for <xref:System.Xml.XmlResolver?qualifyHint=False>.  
  
## How to Fix Violations  
  
-   Catch and process all XmlTextReader exceptions properly to avoid path information disclosure    .  
  
-   Use <xref:System.Xml.XmlSecureResolver?qualifyHint=False>instead of XmlResolver to restrict the resources the XmlTextReader can  access.  
