...
## Remarks  
 `_CrtSetReportHook2` and `_CrtSetReportHookW2` let you hook or unhook a function, whereas [_CrtSetReportHook](../vs140/_CrtSetReportHook.md) only lets you hook a function.  
...
```  
 ...
 These functions validate their parameters. If `mode` or `pfnNewNook` is invalid, these functions invoke the invalid parameter handler, as described in [Parameter Validation](../vs140/Parameter-Validation.md). If execution is allowed to continue, these functions set `errno` to `EINVAL` and return -1.  
