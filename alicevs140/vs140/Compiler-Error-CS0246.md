...
The type or namespace name 'type/namespace' could not be found (are you missing a using directive or an assembly reference?)  
 ...
 A type or namespace that is used in the program was not found. You might have forgotten to reference ([/reference](../vs140/-reference--C#-Compiler-Options-.md)) the assembly that contains the type, or you might not have added the required [using directive](../Topic/using%20Directive%20\(C%23%20Reference\).md).  Or, there might be an issue with the assembly you are trying to reference.  
...
-   If the error is for a namespace name, did you add a reference ([/reference](../vs140/-reference--C#-Compiler-Options-.md)) to the assembly that contains the namespace? For example, your code might contain the directive `using Accessibility`. However, if your project does not reference the assembly Accessibility.dll, error CS0246 is reported. For more information, see [Managing Project References](../Topic/Managing%20references%20in%20a%20project.md)  
  
-   If the error is for a type name, did you include the proper [using](../vs140/using--C#-Reference-.md) directive, or, alternatively, fully qualify the name of the type? Consider the following declaration: `DataSet ds`. To use the `DataSet` type, you need two things. First, you need a reference to the assembly that contains the definition for the `DataSet` type. Second, you need a `using` directive for the namespace where `DataSet` is located. For example, because `DataSet` is located in the **System.Data** namespace, you need the following directive at the beginning of your code: `using System.Data`.  
