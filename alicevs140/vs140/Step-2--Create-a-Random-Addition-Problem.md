...
5.  Add a method that's named `StartTheQuiz()` and that uses the `Random` object's `Next()` method to show the random numbers in the labels. `StartTheQuiz()` will eventually fill in all of the problems and then start the timer, so add a comment. The function should look like the following.  
     ...
     ![Next method](../vs140/media/Express_RandomWhite.png "Express_RandomWhite")  
Next method  
...
8.  Save your code, run it, and then choose the **Start** button.  
     ...
     ![Random addition problem](../vs140/media/Express_AdditionProblem.png "Express_AdditionProblem")  
...
-   To go to the next tutorial step, see [Step 3: Add a Countdown Timer](../vs140/Step-3--Add-a-Countdown-Timer.md).  
  
-   To return to the previous tutorial step, see [Step 1: Create a Project and Add Labels to Your Form](../vs140/Step-1--Create-a-Project-and-Add-Labels-to-Your-Form.md).