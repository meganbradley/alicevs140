...
## Example  
 ...
 The apply function accepts a [std::tuple](../vs140/tuple-Class.md), and produces an integer_sequence by using the  `tuple_size` helper class. Note that [std::decay_t](../vs140/decay-Class.md)_is necessary because [tuple_size](../vs140/tuple_size-Class--tuple-.md) does not work with reference types. The `apply_` function unpacks the tuple members and forwards them as separate arguments to a function call. In this example the function is a simple lambda expression that prints out the values.  
