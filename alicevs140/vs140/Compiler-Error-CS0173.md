...
Type of conditional expression cannot be determined because there is no implicit conversion between 'class1' and 'class2'  
 ...
 To resolve CS0173, verify that there is one and only one implicit conversion between `class1` and `class2`, regardless of which direction the conversion is in and regardless of which class the conversion is in. For more information, see [Implicit Numeric Conversions Table (C# Programmer's Reference)](../vs140/Implicit-Numeric-Conversions-Table--C#-Reference-.md) and [Conversion Operators (C# Programmer's Reference)](../Topic/Conversion%20Operators%20\(C%23%20Programming%20Guide\).md).  
