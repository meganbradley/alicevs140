...
#### Parameters  
 ...
 nCommand  
 [in] The source control command that will be applied to all files in the `lpFileNames` array (see [Command Code Enumerator](../vs140/Command-Code-Enumerator.md) for a list of possible commands).  
...
## Example  
 The IDE builds a list of files that it thinks the user can get. Before it displays this list, it calls the `SccPopulateList` function, giving the source control plug-in the opportunity to add and delete files from the list. The plug-in modifies the list by calling the given callback function (see [Building File Lists Using POPLISTFUNC Function](../vs140/POPLISTFUNC.md) for more details).  
