...
#### Parameters  
 `start_address`  
 Start address of a routine that begins execution of a new thread. For `_beginthread`, the calling convention is either [__cdecl](../vs140/__cdecl.md) (for native code) or [__clrcall](../Topic/__clrcall.md) (for managed code); for `_beginthreadex`, it is either [__stdcall](../vs140/__stdcall.md) (for native code) or [__clrcall](../Topic/__clrcall.md) (for managed code).  
...
## Return Value  
 ...
 If `startaddress` is NULL, the invalid parameter handler is invoked, as described in [Parameter Validation](../vs140/Parameter-Validation.md). If execution is allowed to continue, these functions set `errno` to `EINVAL` and return -1.  
 ...
 For more information about `uintptr_t`, see [Standard Types](../vs140/Standard-Types.md).  
...
-   A thread that's created by using `_beginthreadex` is terminated by a call to [_endthreadex](../vs140/_endthread--_endthreadex.md).  
