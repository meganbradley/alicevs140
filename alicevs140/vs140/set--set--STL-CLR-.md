...
## Remarks  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[begin](../vs140/set--begin--STL-CLR-.md)`(),` `right``.`[end](../vs140/set--end--STL-CLR-.md)`())`, with the default ordering predicate. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the set object `right`, with the default ordering predicate.  
