...
4.  When you start the UI Responsiveness Profiler, a User Account Control window might request your permission to run Visual Studio ETW Collector.exe. Choose **Yes**.  
     ...
     Interact with the app to test the relevant performance scenario. For a detailed workflow, see [Isolate a UI responsiveness issue](#Workflow) and [Isolate a visual throughput problem](#IsolateVisualThroughput).  
...
##  <a name="IsolateAnIssue"></a> Isolate an issue  
 The following section provides suggestions to help you isolate performance problems. For a step-by-step explanation of how to identify and fix performance issues by using a sample performance testing app, see [Walkthrough: Improving Performance (JavaScript)](../vs140/Walkthrough--Improving-UI-responsiveness--HTML-.md).  
...
4.  Optionally, add user marks to your code using [performance.mark](#ProfileMark).  
...
8.  If you have added user marks, they will appear in the [diagnostics session timeline](#Ruler) of the profiler. The following illustration shows a single user mark used to specify a particular operation in your code.  
  
     ![Diagnostics Ruler showing a user mark](../vs140/media/JS_HTMLVizProfiler_UserMark.png "JS_HTMLVizProfiler_UserMark")  
...
9. Identify an area of interest in the timeline and the profiler graphs by using user marks, app lifecycle events, or data visible in the graphs. Here are some guidelines to help you analyze and use the data in the graphs:  
    ...
    -   Use the [diagnostics session timeline](#Ruler) to view [user marks](#ProfileMark), app lifecycle events, and the associated timeline for these events and the timeline for data in the other graphs.  
    ...
    -   If you're developing a game or rich media app, use the [visual throughput (FPS) graph](#VisualThroughput) to identify periods of time in which the frame rate dropped.  
...
10. Select the area of interest in one of the graphs by clicking a part of the graph and dragging the pointer to make a selection (or by using the Tab key and arrow keys). When you select a time period by making a selection, the timeline details graph in the profiler's lower pane changes to show only the selected time period.  
     ...
     ![CPU utilization graph](../vs140/media/JS_HTMLVizProf_CPU_Util.png "JS_HTMLVizProf_CPU_Util")  
  
11. Use the [timeline details graph](#TimelineDetails) to get detailed information about events that are either running too frequently or taking too much time to complete. For example, look for the following:  
...
```  
 ...
 ![User measure event in the timeline details view](../vs140/media/JS_HTMLVizProfiler_User_Measure.png "JS_HTMLVizProfiler_User_Measure")  
...
###  <a name="Ruler"></a> View the diagnostic session timeline  
 ...
 ![Diagnostic session ruler](../vs140/media/JS_HTMLVizProf_Ruler.png "JS_HTMLVizProf_Ruler")  
...
-   The graph shows a value of zero if frames are being dropped.  
 ...
 ![Visual throughput graph](../vs140/media/JS_HTMLVizProf_VizThru.png "JS_HTMLVizProf_VizThru")  
...
>  When an event occurs on a background thread, the thread ID appears in brackets next to the event name.  
 ...
 ![Timeline details graph](../vs140/media/JS_HTMLVizProf_TimelineDet.png "JS_HTMLVizProf_TimelineDet")  
...
###  <a name="FilterTimelineDetails"></a> Filter timeline details  
 ...
 ![Filtering timeline to an event](../vs140/media/JS_HTMLVizProfiler_FilterToEvent.png "JS_HTMLVizProfiler_FilterToEvent")  
...
###  <a name="FilterEvents"></a> Filter events  
 ...
 ![Filtering events in the timeline](../vs140/media/JS_HTMLVizProfiler_Event_Filter.png "JS_HTMLVizProfiler_Event_Filter")  
...
###  <a name="GroupFrames"></a> Group events by frame  
 ...
 ![Group top level events by frame](../vs140/media/JS_HTMLVizProfiler_Frame_Grouping_Button.png "JS_HTMLVizProfiler_Frame_Grouping_Button")  
  
 When you group the events by frame, the top-level events in the timeline details view each represent a frame.  
  
 ![Timeline events grouped by frame](../vs140/media/JS_HTMLVizProfiler_Frame_Grouping.png "JS_HTMLVizProfiler_Frame_Grouping")  
...
## See Also  
 [Analyze app performance](../vs140/Analyze-the-performance-of-Windows-Store-apps-using-Visual-Studio-diagnostic-tools.md)