...
## Remarks  
 The object orders the sequence it controls by calling two stored objects, a comparison function object of type [key_equal](#unordered_multiset__key_equal) and a hash function object of type [hasher](#unordered_multiset__hasher). You access the first stored object by calling the member function [key_eq](#unordered_multiset__key_eq)`()`; and you access the second stored object by calling the member function [hash_function](#unordered_multiset__hash_function)`()`. Specifically, for all values `X` and `Y` of type `Key`, the call `key_eq()(X, Y)` returns true only if the two argument values have equivalent ordering; the call `hash_function()(keyval)` yields a distribution of values of type `size_t`. Unlike template class [unordered_set](../vs140/unordered_set-Class.md), an object of template class `unordered_multiset` does not ensure that `key_eq()(X, Y)` is always false for any two elements of the controlled sequence. (Keys need not be unique.)  
...
### Remarks  
 The member function calls [erase](#unordered_multiset__erase)`(` [begin](#unordered_multiset__begin)`(),` [end](#unordered_multiset__end)`())`.  
...
### Remarks  
 The member function returns the number of elements in the range delimited by [equal_range](#unordered_multiset__equal_range)`(keyval)`.  
...
### Remarks  
 The member function returns [equal_range](#unordered_multiset__equal_range)`(keyval).first`.  
...
|`ValTy`|Template parameter that specifies the argument type that the unordered_multiset can use to construct an element of [value_type](../vs140/map-Class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Remarks  
 ...
 The [value_type](../vs140/map-Class.md#map__value_type) of a container is a typedef that belongs to the container, and, for set, `unordered_multiset<V>::value_type` is type `const V`.  
...
### Remarks  
 The member function returns `(float)`[size](#unordered_multiset__size)`() / (float)`[bucket_count](#unordered_multiset__bucket_count)`()`, the average number of elements per bucket.  
