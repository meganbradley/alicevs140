...
-   Every update to a model or diagram must be made in the user interface (UI) thread. The background thread must use <xref:System.Windows.Forms.Control.Invoke?qualifyHint=False> or `Dispatcher.`<xref:System.Windows.Threading.Dispatcher.Invoke``1?qualifyHint=False> to have the UI thread perform the actual updates.  
...
1.  Create a command handler in a C# project as described in [How to Define a Menu Command on a Modeling Diagram](../Topic/Define%20a%20menu%20command%20on%20a%20modeling%20diagram.md).  
...
## See Also  
 ...
 [How to Define a Gesture Handler on a Modeling Diagram](../Topic/Define%20a%20gesture%20handler%20on%20a%20modeling%20diagram.md)