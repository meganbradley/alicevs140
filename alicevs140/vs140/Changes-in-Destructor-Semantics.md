...
```  
 ...
 While this implementation allows the user to explicitly invoke the class `Finalize` method now rather than at a time you have no control over, it does not really tie in with the `Dispose` method solution. This is changed in [!INCLUDE[cpp_current_long](../vs140/includes/cpp_current_long_md.md)].  
  
## Destructors in New Syntax  
 In the new syntax, the destructor is renamed internally to the `Dispose` method and the reference class is automatically extended to implement the `IDispose` interface. That is, under [!INCLUDE[cpp_current_long](../vs140/includes/cpp_current_long_md.md)], our pair of classes is transformed as follows:  
