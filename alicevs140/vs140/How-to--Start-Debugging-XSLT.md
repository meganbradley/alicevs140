...
## XSLT from Other Languages  
 You can also step into XSLT while debugging an application. When you press F11 on an <xref:System.Xml.Xsl.XslCompiledTransform.Transform?qualifyHint=True> call, the debugger can step into the XSLT code.  
  
> [!NOTE]
>  Stepping into XSLT from the <xref:System.Xml.Xsl.XslTransform?qualifyHint=False> class is not supported. The <xref:System.Xml.Xsl.XslCompiledTransform?qualifyHint=False> class is the only XSLT processor that supports stepping into XSLT while debugging.  
  
#### To start debugging an XSLT application  
  
1.  When instantiating the <xref:System.Xml.Xsl.XslCompiledTransform?qualifyHint=False> object, set the `enableDebug` parameter to `true` in your code.  
