...
##  <a name="vcconjumpingoutofafinallyblock"></a> Jumping Out of a Finally Block  
 In native C/C++ code, jumping out of a __**finally** block using structured exception handling (SEH) is allowed although it produces a warning.  Under [/clr](../Topic/-clr%20\(Common%20Language%20Runtime%20Compilation\).md), jumping out of a **finally** block causes an error:  
...
##  <a name="vcconexceptionfiltersandexception_continue_execution"></a> Exception Filters and EXCEPTION_CONTINUE_EXECUTION  
 If a filter returns `EXCEPTION_CONTINUE_EXECUTION` in a managed application, it is treated as if the filter returned `EXCEPTION_CONTINUE_SEARCH`. For more information on these constants, see [try-except Statement](../vs140/try-except-Statement.md).  
