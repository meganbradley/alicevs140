...
|Name|Description|  
|----------|-----------------|  
|[CAdapt::operator const T&](../vs140/CAdapt--operator-const-T-.md)|Returns a `const` reference to `m_T`.|  
|[CAdapt::operator T&](../vs140/CAdapt--operator-T-.md)|Returns a reference to `m_T`.|  
|[CAdapt::operator <](../vs140/CAdapt--operator--.md)|Compares an object of the adapted type with `m_T`.|  
|[CAdapt::operator =](../vs140/CAdapt--operator-=.md)|Assigns an object of the adapted type to `m_T`.|  
|[CAdapt::operator ==](../vs140/CAdapt--operator-==.md)|Compares an object of the adapted type with `m_T`.|  
...
## Remarks  
 ...
 `CAdapt`'s primary role is to hide the address-of operator defined by class `T`, yet still retain the characteristics of the adapted class. `CAdapt` fulfils this role by holding a public member, [m_T](../vs140/CAdapt--m_T.md), of type `T`, and by defining conversion operators, comparison operators, and a copy constructor to allow specializations of `CAdapt` to be treated as if they are objects of type `T`.  
 ...
 Typically, you will use `CAdapt` when you want to store `CComBSTR`, `CComPtr`, `CComQIPtr`, or `_com_ptr_t` objects in a container-style class. This was most commonly necessary for C++ Standard Library containers prior to support for the C++11 Standard, but C++11 Standard Library containers automatically work with types that have overloaded `operator&()`. The Standard Library achieves this by internally using [std::addressof()](../vs140/addressof.md) to get the true addresses of objects.  
