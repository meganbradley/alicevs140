...
## Assemblies tab  
 ...
 All desktop projects contain an implicit reference to mscorlib. [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)] projects contain an implicit reference to Microsoft.VisualBasic. In [!INCLUDE[vs_dev11_long](../vs140/includes/vs_dev11_long_md.md)], all projects contain an implicit reference to System.Core, even if it’s removed from the list of references.  
...
2.  Extensions lists all assemblies that external vendors of components and controls have developed to extend the targeted Framework. Depending on the purpose of the user application, it might need these assemblies.  
 ...
 Some components in the list may not be shown, depending on the [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] version of your project. This can occur under the following conditions:  
...
-   A component that uses a recent version of the .NET Framework is incompatible with a project that targets an earlier version of the .NET Framework.  
     ...
     For information about how to change the target .NET Framework version for a project, see [How to: Target a Specific .NET Framework Version or Profile](../vs140/How-to--Target-a-Version-of-the-.NET-Framework.md).  
  
-   A component that uses [!INCLUDE[net_v40_short](../vs140/includes/net_v40_short_md.md)] is incompatible with a project that targets the [!INCLUDE[net_v45](../vs140/includes/net_v45_md.md)].  
  
     When you create a new application, some projects target the [!INCLUDE[net_v45](../vs140/includes/net_v45_md.md)] by default. For more information, see [.NET Framework Client Profile](assetId:///f0219919-1f02-4588-8704-327a62fd91f1).  
  
-   You should avoid adding file references to outputs of another project in the same solution, because doing this may cause compilation errors. Instead, use the **Projects** tab of the **Add Reference** dialog box to create project-to-project references. This makes team development easier by enabling better management of the class libraries you create in your projects. For more information, see [Troubleshooting Broken References](../vs140/Troubleshooting-Broken-References.md).  
...
## Solution tab  
 ...
 A project can reference another project that targets a different version of the .NET Framework. For example, you could create a project that targets the [!INCLUDE[net_v40_short](../vs140/includes/net_v40_short_md.md)] but that references an assembly that’s been built for the .NET Framework 2. However, the .NET Framework 2 project can’t reference a [!INCLUDE[net_v40_short](../vs140/includes/net_v40_short_md.md)] project. For more information, see [Targeting a Specific .NET Framework Version or Profile](../vs140/Targeting-a-Specific-.NET-Framework-Version.md).  
  
 A project that targets the [!INCLUDE[net_v40_short](../vs140/includes/net_v40_short_md.md)] is incompatible with a project that targets the [!INCLUDE[net_client_v40_long](../vs140/includes/net_client_v40_long_md.md)].  
  
 In [!INCLUDE[vs_dev11_long](../vs140/includes/vs_dev11_long_md.md)], a file reference instead of a project reference is created if one project targets the .NET Framework 4 and another project targets an earlier version.  
  
 A project that targets the [!INCLUDE[net_win8_profile](../vs140/includes/net_win8_profile_md.md)] can’t add a project reference to a project that targets the .NET Framework and vice versa.  
...
### Core Subgroup  
 ...
 [!INCLUDE[win8_appname_long](../vs140/includes/win8_appname_long_md.md)] app projects contain references to all of the WinMDs in the [!INCLUDE[win8](../vs140/includes/win8_md.md)] SDK by default on project creation. In managed projects, a read-only node under the References folder in **Solution Explorer** indicates the reference to the entire [!INCLUDE[win8](../vs140/includes/win8_md.md)] SDK. Accordingly, the Core subgroup in the Reference Manager won’t enumerate any of the assemblies from the [!INCLUDE[win8](../vs140/includes/win8_md.md)] SDK and instead displays a message: “The Windows SDK is already referenced. Please use the Object Browser to explore the references in the Windows SDK.”  
...
## Browse button  
 ...
 A project can reference a component that targets a different version of the .NET Framework. For example, you could create an application that targets the .NET Framework 4 Client Profile, which references a component that targets the .NET Framework 2. For more information, see [Targeting a Specific .NET Framework Version or Profile](../vs140/Targeting-a-Specific-.NET-Framework-Version.md).  
