...
This example builds on the previous example, [Retrieving the Text of the Paragraphs (C#)](../Topic/Retrieving%20the%20Text%20of%20the%20Paragraphs%20\(C%23\).md), by refactoring the concatenation of strings using a pure function that is implemented as an extension method.  
  
 The previous example used the <xref:System.Linq.Enumerable.Aggregate``3?qualifyHint=False> standard query operator to concatenate multiple strings into one string. However, it is more convenient to write an extension method to do this, because the resulting query smaller and more simple.  
