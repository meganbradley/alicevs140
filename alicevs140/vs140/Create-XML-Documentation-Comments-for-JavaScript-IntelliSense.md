...
-   In a constructor function or object definition, add a [<field\>](../vs140/-field---JavaScript-.md) element preceded by three slash marks (///).  
     ...
     The following example shows the use of the `<field>` element in a constructor function. For additional examples, see [<field\>](../vs140/-field---JavaScript-.md).  
...
1.  In the function, add a [<signature\>](../vs140/-signature---JavaScript-.md) element for each overload. In these elements, add other elements, such as `<summary>`, `<param>`, and `<returns>`, preceding each element with three slash marks (///).  
...
1.  Create an XML file that has documentation comments in the OpenAjax MessageBundle format.  
    ...
    > [!IMPORTANT]
    >  MessageBundle is the recommended format. This format is not supported in Microsoft Ajax or in .winmd files. For information about using the alternative `VSDoc` format, see [<loc\>](../vs140/-loc---JavaScript-.md).  
...
2.  In your .js file, add the following code. The `<loc>` element must be declared before any script, and follows the same usage rules as the `<reference>` element. For more information, see [JavaScript IntelliSense](../Topic/JavaScript%20IntelliSense.md) and [<loc\>](../vs140/-loc---JavaScript-.md).  
...
## See Also  
 [JavaScript IntelliSense Overview](../Topic/JavaScript%20IntelliSense.md)   
