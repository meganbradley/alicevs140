...
### Remarks  
 ...
 You can use this member function in place of the [end()](../vs140/-iterator--functions.md#end) template function to guarantee that the return value is `const_iterator`. Typically, it's used in conjunction with the [auto](../vs140/auto--C---.md) type deduction keyword, as shown in the following example. In the example, consider `Container` to be a modifiable (non- `const`) container or `initializer_list` of any kind that supports `end()` and `cend()`.  
...
### Remarks  
 The template function returns [insert_iterator](../vs140/insert_iterator-Class.md#insert_iterator__insert_iterator)`<Container>(``_Cont``,` `_Where``)`.  
