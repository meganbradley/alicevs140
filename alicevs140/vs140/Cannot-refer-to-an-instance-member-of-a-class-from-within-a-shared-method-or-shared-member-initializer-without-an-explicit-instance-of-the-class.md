...
```  
 ...
 The variable `x` is an instance member because it is not declared as [Shared](../Topic/Shared%20\(Visual%20Basic\).md). Each instance of class `sample` contains its own individual variable `x`. When one instance sets or changes the value of `x`, it does not affect the value of `x` in any other instance.  
