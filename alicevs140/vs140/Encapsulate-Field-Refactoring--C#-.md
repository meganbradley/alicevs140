...
The **Encapsulate Field** refactoring operation enables you to quickly create a property from an existing field, and then seamlessly update your code with references to the new property.  
 ...
 When a [field](../vs140/Fields--C#-Programming-Guide-.md) is [public](../vs140/public--C#-Reference-.md), other objects have direct access to that field and can modify it, undetected by the object that owns that field. By using [properties](../vs140/Properties--C#-Programming-Guide-.md) to encapsulate that field, you can disallow direct access to fields.  
  
 To create the new property, the **Encapsulate Field** operation changes the access modifier for the field that you want to encapsulate to [private](../vs140/private--C#-Reference-.md), and then generates [get](../vs140/get--C#-Reference-.md) and [set](../vs140/set--C#-Reference-.md) accessors for that field. In some cases, only a `get` accessor is generated, such as when the field is declared read-only.  
...
2.  In the [Code Editor](../vs140/Writing-Code-in-the-Code-and-Text-Editor.md), place the cursor in the declaration, on the name of the field that you want to encapsulate. In the example below, place the cursor on the word `width`:  
