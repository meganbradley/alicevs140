...
```  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[begin](#array__begin)`(),` `right``.`[end](#array__end)`())`. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the array object `right`.  
...
### Remarks  
 `crend` is used with a reversed array just as [cend](#array__cend) is used with a array.  
