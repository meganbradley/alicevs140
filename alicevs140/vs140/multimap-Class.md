...
### Remarks  
 `crend` is used with a reversed `multimap` just as [end](#multimap__end) is used with a `multimap`.  
...
|`Where`|The place to start searching for the correct point of insertion. (If that point immediately precedes `Where`, insertion can occur in amortized constant time instead of logarithmic time.)|  
|`ValTy`|Template parameter that specifies the argument type that the map can use to construct an element of [value_type](../vs140/map-Class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Remarks  
 ...
 For more information on `Type` see the [multimap Class](../vs140/multimap-Class.md) topic.  
...
### Remarks  
 `rend` is used with a reversed multimap just as [end](../vs140/map-Class.md#map__end) is used with a multimap.  
