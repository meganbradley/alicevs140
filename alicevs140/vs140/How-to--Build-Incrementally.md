...
>  A target in which there is no direct mapping between the inputs and outputs will always build more often than a target in which each output can map to only one input because [!INCLUDE[vstecmsbuild](../vs140/includes/vstecmsbuild_md.md)] cannot determine which outputs need to be rebuilt if some of the inputs have changed.  
 ...
 Tasks in which you can identify a direct mapping between the outputs and inputs, such as the [LC Task](../Topic/LC%20Task.md), are most suitable for incremental builds, unlike tasks such as `Csc` and [Vbc](../Topic/Vbc%20Task.md), which produce one output assembly from a number of inputs.  
...
-   `BuildHelp`: Combines .content files and XML metadata files to build the final .help file.  
 ...
 The project uses transforms to create a one-to-one mapping between inputs and outputs in the `GenerateContentFiles` task. For more information, see [MSBuild Transforms](../vs140/MSBuild-Transforms.md). Also, the `Output` element is set to automatically use the outputs from the `GenerateContentFiles` task as the inputs for the `BuildHelp` task.  
