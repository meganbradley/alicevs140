...
### Accessing the Visual Studio API  
 Setting the `hostspecific` attribute of the `<#@template#>` directive allows the template to obtain access to the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] API. The template can use this to obtain the location of the project files, to avoid using an absolute file path in the template code.  
...
## Troubleshooting the Text Template  
 If you have seen template transformation or compilation errors in the **Error List**, or if the output file was not generated correctly, you can troubleshoot the text template with the techniques described in [Troubleshooting Text Templates](../Topic/Generating%20Files%20with%20the%20TextTransform%20Utility.md).  
