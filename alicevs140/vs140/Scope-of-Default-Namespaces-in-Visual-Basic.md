...
Default namespaces as represented in the XML tree are not in scope for queries. If you have XML that is in a default namespace, you still must declare an <xref:System.Xml.Linq.XNamespace?qualifyHint=False> variable, and combine it with the local name to make a qualified name to be used in the query.  
