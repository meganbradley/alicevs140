...
-   **Control blocks** - program code that inserts variable values into the text, and controls conditional or repeated parts of the text.  
 ...
 To try the examples in this topic, copy them into a template file as described in [Generating Application Code and Files from Templates](../vs140/Design-Time-Code-Generation-by-using-T4-Text-Templates.md). After editing the template file, save it, and then inspect the output **.txt** file.  
...
```  
 ...
 For more information about directives, see [Text Template Directives](../vs140/T4-Text-Template-Directives.md).  
...
## Control blocks  
 Control blocks are sections of program code that are used to transform the templates. The default language is C#, but to use [!INCLUDE[vbprvb](../vs140/includes/vbprvb_md.md)], you can write this directive at the beginning of the file:  
...
```  
 ...
 For more information about control blocks, see [Text Template Control Blocks](../vs140/Text-Template-Control-Blocks.md).  
...
```  
 ...
 For a list of macros, see [Macros for Build Commands and Properties](../Topic/Common%20Macros%20for%20Build%20Commands%20and%20Properties.md).  
  
 The assembly directive has no effect in a [preprocessed text template](../vs140/Run-Time-Text-Generation-with-T4-Text-Templates.md).  
  
 For more information, see [Assembly Directive](../vs140/T4-Assembly-Directive.md).  
...
```  
 ...
 For more information, see [Import Directive](../vs140/T4-Import-Directive.md).  
...
###  <a name="Include"></a> Including code and text  
 ...
 For more information, see [Include Directive](../vs140/T4-Include-Directive.md).  
...
### Utility methods  
 ...
 For more information, see [Text Template Utility Functions](../vs140/Text-Template-Utility-Methods.md).  
 ...
 **Edit the model file in a diagram or form.** [!INCLUDE[dsl](../vs140/includes/dsl_md.md)] provides tools that let you edit a model as a diagram or Windows form. This makes it easier to discuss the model with users of the generated application. [!INCLUDE[dsl](../vs140/includes/dsl_md.md)] also creates a set of strongly-typed classes that reflect the structure of the model. For more information, see [Using Text Templates in Domain-Specific Language Solutions](../vs140/Generating-Code-from-a-Domain-Specific-Language.md).  
  
 **Use a UML model**. You can generate code from a UML model. This has the advantage that the model can be edited as a diagram in a familiar notation. Also, you do not have to design the diagram. For more information, see [How to Generate Files from a UML Model](../vs140/Generate-files-from-a-UML-model.md).  
  
### Relative file paths in design-time templates  
 In a [design-time text template](../vs140/Design-Time-Code-Generation-by-using-T4-Text-Templates.md), if you want to reference a file in a location relative to the text template, use `this.Host.ResolvePath()`. You must also set `hostspecific="true"` in the `template` directive:  
...
### Design-time Text Templates run in a separate AppDomain  
 You should be aware that a [design-time text template](../vs140/Design-Time-Code-Generation-by-using-T4-Text-Templates.md) runs in an AppDomain that is separate from the main application. In most cases this is not important, but you might discover restrictions in certain complex cases. For example, if you want to pass data in or out of the template from a separate service, then the service must provide a serializable API.  
  
 (This isn’t true of a [run-time text template](../vs140/Run-Time-Text-Generation-with-T4-Text-Templates.md), which provides code that is compiled along with the rest of your code.)  
...
|Task|Topic|  
|----------|-----------|  
|Writing a template.|[Guidelines for Writing Text Templates](../Topic/Guidelines%20for%20Writing%20T4%20Text%20Templates.md)|  
|Generate text by using program code.|[Text Template Structure](../vs140/Writing-a-T4-Text-Template.md)|  
...
|Run text generation outside [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].|[The Text Transform Executable](../Topic/Generating%20Files%20with%20the%20TextTransform%20Utility.md)|  
