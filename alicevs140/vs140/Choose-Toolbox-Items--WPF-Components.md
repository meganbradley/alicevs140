...
## Limitations  
 Adding a custom control or <xref:System.Windows.Controls.UserControl?qualifyHint=False> to the Toolbox has the following limitations.  
...
-   Does not update correctly when you change the solution configuration from Debug to Release, or from Release to Debug. This is because the reference is not a project reference, but is for the assembly on disk instead. If the control is part of the current solution, when you change from Debug to Release, your project continues to reference the Debug version of the control.  
 ...
 In addition, if design-time metadata is applied to the custom control and this metadata specifies that the <xref:Microsoft.Windows.Design.ToolboxBrowsableAttribute?qualifyHint=False> is set to `false`, the control does not appear in the Toolbox.  
