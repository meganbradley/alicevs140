...
### Heap-based lifetime  
 For heap object lifetime, use [smart pointers](../vs140/Smart-Pointers--Modern-C---.md). Use `shared_ptr` and `make_shared` as the default pointer and allocator. Use `weak_ptr` to break cycles, do caching, and observe objects without affecting or assuming anything about their lifetimes.  
...
```  
 ...
 Use `unique_ptr` for unique ownership, for example, in the *pimpl* idiom. (See [Pimpl For Compile-Time Encapsulation](../vs140/Pimpl-For-Compile-Time-Encapsulation--Modern-C---.md).) Make a `unique_ptr` the primary target of all explicit `new` expressions.  
