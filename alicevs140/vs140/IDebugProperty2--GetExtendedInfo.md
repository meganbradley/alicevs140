...
#### Parameters  
 ...
 `pExtendedInfo`  
 [out] Returns a `VARIANT` (C++) or object (C#) that can be used to retrieve the extended property information. For example, this parameter might return an `IUnknown` interface that can be queried for an [IDebugDocumentText2](../Topic/IDebugDocumentText2.md) interface. See Remarks for details.  
...
## Remarks  
 This method exists for the purpose of retrieving information that does not lend itself to being retrieved by calling the [GetPropertyInfo](../vs140/IDebugProperty2--GetPropertyInfo.md) method.  
...
|----------|----------|-----------------|  
|guidDocument|{3f98de84-fee9-11d0-b47f-00a0244a1dd2}|Returns an `IUnknown` interface to the document. Typically, the [IDebugDocumentText2](../Topic/IDebugDocumentText2.md) interface can be obtained from this `IUnknown` interface.|  
|guidCodeContext|{e2fc65e-56ce-11d1-b528-00aax004a8797}|Returns an `IUnknown` interface to the document context. Typically, the [IDebugDocumentContext2](../vs140/IDebugDocumentContext2.md) interface can be obtained from this `IUnknown` interface.|  
