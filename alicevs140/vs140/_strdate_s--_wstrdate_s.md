...
## Remarks  
 ...
 If `buffer` is a `NULL` pointer, or if `numberOfElements` is less than 9 characters, the invalid parameter handler is invoked, as described in [Parameter Validation](../vs140/Parameter-Validation.md). If execution is allowed to continue, these functions return -1 and set `errno` to `EINVAL` if the buffer is `NULL` or if `numberOfElements` is less than or equal to 0, or set `errno` to `ERANGE` if `numberOfElements` is less than 9.  
