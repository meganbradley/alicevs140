...
Describes an object that controls insertion and extraction of elements and encoded objects using a stream buffer of class [basic_stringbuf](../vs140/basic_stringbuf-Class.md)< **Elem**, **Tr**, `Alloc`>.  
...
|||  
|-|-|  
|[rdbuf](#basic_stringstream__rdbuf)|Returns the address of the stored stream buffer of type `pointer` to [basic_stringbuf](../vs140/basic_stringbuf-Class.md)< `Elem`, `Tr`, `Alloc`>.|  
...
### Remarks  
 The first member function returns [rdbuf](#basic_stringstream__rdbuf) -> [str](../vs140/basic_stringbuf-Class.md#basic_stringbuf__str). The second member function calls `rdbuf` -> **str**( `_Newstr`).  
