...
#### Parameters  
 ...
 `pCallback`  
 [in] The [IDebugEventCallback2](../Topic/IDebugEventCallback2.md) object that receives debugger events.  
...
## Remarks  
 Normally, [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] launches a program using the [IDebugPortEx2::LaunchSuspended](../vs140/IDebugPortEx2--LaunchSuspended.md) method and then attaches the debugger to the suspended program. However, there are circumstances in which the debug engine may need to launch a program (for example, if the debug engine is part of an interpreter and the program being debugged is an interpreted language), in which case [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] uses the `IDebugEngineLaunch2::LaunchSuspended` method.  
