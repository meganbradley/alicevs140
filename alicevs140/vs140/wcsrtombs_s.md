...
#### Parameters  
 ...
 [in] `count`  
 The maximum number of bytes to be stored in the `mbstr` buffer, or [_TRUNCATE](../vs140/_TRUNCATE.md).  
...
-   The number of bytes stored in the `mbstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../vs140/_TRUNCATE.md), then `wcsrtombs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
