...
1.  If you are defining any new menus, submenus, shortcut menus, or toolbars, add a `Menus` element to the `Commands` element. Then, for each menu to be created, add a [Menu](../Topic/Menu%20Element.md) element to the `Menus` element.  
...
3.  If you are adding new commands to the IDE, add a `Buttons` element to the `Commands` element. Then, for each command, add a [Button](../vs140/Button-Element.md) element to the `Buttons` element.  
    ...
    3.  If your command will have an icon, create an [Icon](../vs140/Icon-Element.md) element in the `Button` element, and set its `guid` and `id` attributes to the `Bitmap` element for the icon.  
  
        > [!NOTE]
        >  Toolbar buttons must have icons.  
  
     For more information, see [How to: Create and Handle Commands in VSPackages (C#)](../Topic/MenuCommands%20Vs.%20OleMenuCommands.md).  
...
7.  To localize menu text, use the `LocCanonicalName` element. For more information, see [Strings](../vs140/Strings-Element.md).  
 ...
 Some menu and button types include specialized behaviors. The following table describes some specialized menu and button types. For other types, see the `types` attribute descriptions in [Menu](../Topic/Menu%20Element.md), [Button](../vs140/Button-Element.md), and [Combo Element](../vs140/Combo-Element.md).  
