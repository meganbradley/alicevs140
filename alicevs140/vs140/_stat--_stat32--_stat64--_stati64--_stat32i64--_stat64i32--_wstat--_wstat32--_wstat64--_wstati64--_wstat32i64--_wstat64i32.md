...
> [!NOTE]
>  `_wstat` does not work with [!INCLUDE[wiprlhext](../vs140/includes/wiprlhext_md.md)] symbolic links. In these cases, `_wstat` will always report a file size of 0. `_stat` does work correctly with symbolic links.  
  
 This function validates its parameters. If either `path` or `buffer` is `NULL`, the invalid parameter handler is invoked, as described in [Parameter Validation](../vs140/Parameter-Validation.md).  
...
|`_tstat64i32`|`_stat64i32`|`_stat64i32`|`_wstat64i32`|  
 ...
 If `path` refers to a device, the `st_size`, various time fields, `st_dev`, and `st_rdev` fields in the `_stat` structure are meaningless. Because STAT.H uses the [_dev_t](../vs140/Standard-Types.md) type that is defined in TYPES.H, you must include TYPES.H before STAT.H in your code.  
