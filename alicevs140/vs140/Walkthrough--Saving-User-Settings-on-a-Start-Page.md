...
You can persist user settings for your start page. By following this walkthrough, you can create a control that saves a setting to the registry when the user clicks a button, and then retrieves that setting every time the Start Page loads. Because the Start Page project template includes a customizable user control, and the default Start Page XAML calls that control, you do not have to modify the Start Page itself.  
 ...
 The settings store that is instantiated in this walkthrough is an instance of the <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore?qualifyHint=False> interface, which reads and writes to the following registry location when it is called: HKCU\Software\Microsoft\VisualStudio\14.0\\*CollectionName*  
...
1.  Create a Start Page project by using the Start Page project template, as described in [Start Pages](../Topic/Creating%20Your%20Own%20Start%20Page.md). Name the project **SaveMySettings**.  
...
4.  From the XAML pane, in the top-level <xref:System.Windows.Controls.UserControl?qualifyHint=False> element definition, add the following event declaration after the namespace declarations.  
...
5.  In the design pane, click the main area of the control, and then press DELETE.  
     ...
     This removes the <xref:System.Windows.Controls.Border?qualifyHint=False> element and everything in it, and leaves only the top-level <xref:System.Windows.Controls.Grid?qualifyHint=False> element.  
  
6.  From the **Toolbox**, drag a <xref:System.Windows.Controls.StackPanel?qualifyHint=False> control to the grid.  
  
7.  Now drag a <xref:System.Windows.Controls.TextBlock?qualifyHint=False>, a <xref:System.Windows.Controls.TextBox?qualifyHint=False>, and a Button to the <xref:System.Windows.Controls.StackPanel?qualifyHint=False>.  
  
8.  Add an **x:Name** attribute for the <xref:System.Windows.Controls.TextBox?qualifyHint=False>, and a `Click` event for the <xref:System.Windows.Controls.Button?qualifyHint=False>, as shown in the following example.  
...
1.  In the XAML pane, right-click the `Click` attribute of the <xref:System.Windows.Controls.Button?qualifyHint=False> element, and then click **Navigate to Event Handler**.  
...
2.  Add the following `using` statements to the top of the file.  
     ...
     [!CODE [StartPageDTE#11](../CodeSnippet/VS_Snippets_VSSDK/startpagedte#11)]  
...
3.  Add a private `SettingsStore` property, as shown in the following example.  
    ...
    ```  
     ...
     This property first gets a reference to the <xref:EnvDTE80.DTE2?qualifyHint=False> interface, which contains the Automation object model, from the <xref:System.Windows.FrameworkElement.DataContext?qualifyHint=False> of the user control, and then uses the DTE to get an instance of the <xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsManager?qualifyHint=False> interface. Then it uses that instance to return the current user settings.  
