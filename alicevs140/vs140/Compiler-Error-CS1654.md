...
Cannot modify members of 'variable' because it is a 'read-only variable type'  
 ...
 One common area that this occurs is within [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md) loops. It is a compile-time error to modify the value of the collection elements. Therefore, you cannot make any modifications to elements that are [value types](../Topic/Value%20Types%20\(C%23%20Reference\).md), including [structs](../vs140/Structs--C#-Programming-Guide-.md). In a collection whose elements are [reference types](../vs140/Reference-Types--C#-Reference-.md), you can modify accessible members of each element, but any try to add or remove or replace complete elements will generate [Compiler Error CS1656](../vs140/Compiler-Error-CS1656.md).  
  
## Example  
 The following example generates error CS1654 because `Book` is a `struct`. To fix the error, change the `struct` to a [class](../vs140/class--C#-Reference-.md).  
