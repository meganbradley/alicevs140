...
```  
 ...
 The <xref:System.Linq.Enumerable.Where``1?qualifyHint=False> method is an extension method. For more information, see [Extension Methods](../Topic/Extension%20Methods%20\(C%23%20Programming%20Guide\).md). Because <xref:System.Linq.Enumerable.Where``1?qualifyHint=False> is an extension method, the query above is compiled as though it were written as follows:  
...
## Executing XPath Expressions with XmlDocument  
 The following example uses <xref:System.Xml.XmlDocument?qualifyHint=False> to accomplish the same results as the previous examples:  
...
```  
 ...
 This query returns the same output as the examples that use LINQ to XML; the only difference is that LINQ to XML indents the printed XML, whereas <xref:System.Xml.XmlDocument?qualifyHint=False> does not.  
  
 However, the <xref:System.Xml.XmlDocument?qualifyHint=False> approach generally does not perform as well as LINQ to XML, because the <xref:System.Xml.XmlNode.SelectNodes?qualifyHint=False> method must do the following internally every time it is called:  
