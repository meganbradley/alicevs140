...
## Cause  
 Code that is marked as <xref:System.Security.SecurityTransparentAttribute?qualifyHint=False> is not granted sufficient permissions to assert.  
  
## Rule Description  
 This rule analyzes all methods and types in an assembly which is either 100% transparent or mixed transparent/critical, and flags any declarative or imperative usage of <xref:System.Security.CodeAccessPermission.Assert?qualifyHint=False>.  
  
 At run time, any calls to <xref:System.Security.CodeAccessPermission.Assert?qualifyHint=False> from transparent code will cause a <xref:System.InvalidOperationException?qualifyHint=False> to be thrown. This can occur in both 100% transparent assemblies, and also in mixed transparent/critical assemblies where a method or type is declared transparent, but includes a declarative or imperative Assert.  
  
 The [!INCLUDE[dnprdnshort](../vs140/includes/dnprdnshort_md.md)] 2.0 introduced a feature named *transparency*. Individual methods, fields, interfaces, classes, and types can be either transparent or critical.  
...
## How to Fix Violations  
 To resolve the issue, either mark the code which calls the Assert with the <xref:System.Security.SecurityCriticalAttribute?qualifyHint=False>, or remove the Assert.  
  
## When to Suppress Warnings  
 Do not suppress a message from this rule.  
  
## Example  
 This code will fail if `SecurityTestClass` is transparent, when the `Assert` method throws a <xref:System.InvalidOperationException?qualifyHint=False>.  
...
## See Also  
 [Security Warnings](../vs140/Security-Warnings.md)