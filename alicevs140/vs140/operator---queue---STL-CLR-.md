...
## Remarks  
 The operator function returns true if, for the lowest position `i` for which `!(``right``[i] <` `left``[i])` it is also true that `left``[i] <` `right``[i]`. Otherwise, it returns `left``->`[size](../vs140/queue--size--STL-CLR-.md)`() <` `right``->size()` You use it to test whether `left` is ordered before `right` when the two queues are compared element by element.  
