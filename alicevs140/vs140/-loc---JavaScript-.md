...
## Remarks  
 The `<loc>` element must appear at the top of the file in the same section as the `<reference>` element. Usage rules for the `<loc>` element are the same as the `<reference>` element. For more information, see the “References Directives” section in [JavaScript IntelliSense](../Topic/JavaScript%20IntelliSense.md).  
