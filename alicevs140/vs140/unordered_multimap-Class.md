...
## Remarks  
 The object orders the sequence it controls by calling two stored objects, a comparison function object of type [key_equal](#unordered_multimap__key_equal) and a hash function object of type [hasher](#unordered_multimap__hasher). You access the first stored object by calling the member function [key_eq](#unordered_multimap__key_eq)`()`; and you access the second stored object by calling the member function [hash_function](#unordered_multimap__hash_function)`()`. Specifically, for all values `X` and `Y` of type `Key`, the call `key_eq()(X, Y)` returns true only if the two argument values have equivalent ordering; the call `hash_function()(keyval)` yields a distribution of values of type `size_t`. Unlike template class [unordered_map](../vs140/unordered_map-Class.md), an object of template class `unordered_multimap` does not ensure that `key_eq()(X, Y)` is always false for any two elements of the controlled sequence. (Keys need not be unique.)  
  
 The object also stores a maximum load factor, which specifies the maximum desired average number of elements per bucket. If inserting an element causes [load_factor](#unordered_multimap__load_factor)`()` to exceed the maximum load factor, the container increases the number of buckets and rebuilds the hash table as needed.  
...
### Remarks  
 The member function calls [erase](#unordered_multimap__erase)`(` [begin](#unordered_multimap__begin)`(),` [end](#unordered_multimap__end)`())`.  
...
### Remarks  
 The member function returns the number of elements in the range delimited by [equal_range](#unordered_multimap__equal_range)`(keyval)`.  
...
### Remarks  
 The member function returns [equal_range](#unordered_multimap__equal_range)`(keyval).first`.  
...
|`ValTy`|Template parameter that specifies the argument type that the unordered_multimap can use to construct an element of [value_type](../vs140/map-Class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Remarks  
 The member function returns `(float)`[size](#unordered_multimap__size)`() / (float)`[bucket_count](#unordered_multimap__bucket_count)`()`, the average number of elements per bucket.  
