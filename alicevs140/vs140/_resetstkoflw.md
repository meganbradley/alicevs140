...
-   A `__finally` block.  
 ...
 Structured exception handling will not catch a `STATUS_STACK_OVERFLOW` exception when the application is compiled with `/clr` or `/clr:pure` (see [/clr topic](../Topic/-clr%20\(Common%20Language%20Runtime%20Compilation\).md)).  
