...
Correct execution sometimes depends on the value that the last iteration of a loop assigns to a variable. Such programs must list all such variables as arguments to a `lastprivate` clause ([Section 2.7.2.3](../vs140/2.7.2.3-lastprivate.md) on page 27) so that the values of the variables are the same as when the loop is executed sequentially.  
