...
## Obtaining access to the host  
 Set `hostspecific="true"` in the `template` directive. This lets you use  `this.Host`, which has type <xref:Microsoft.VisualStudio.TextTemplating.ITextTemplatingEngineHost?qualifyHint=False>. This type has members that you can use, for example, to resolve file names and to log errors.  
...
## Using the Visual Studio API  
 If you are executing a text template in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], you can use `this.Host` to access services provided by [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] and any packages or extensions that are loaded.  
  
 Set hostspecific="true" and cast `this.Host` to <xref:System.IServiceProvider?qualifyHint=False>.  
  
 This example gets the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] API, <xref:EnvDTE.DTE?qualifyHint=False>, as a service:  
