...
### Remarks  
 To begin the thread's execution, call the [CreateThread](#cwinthread__createthread) member function. You will usually create threads by calling [AfxBeginThread](../vs140/AfxBeginThread.md), which will call this constructor and `CreateThread`.  
...
### Return Value  
 This function returns a pointer to one of two types of windows. If your thread is part of an OLE server and has an object that is in-place active inside an active container, this function returns the [CWinApp::m_pActiveWnd](../vs140/CWinApp-Class.md#cwinapp__m_pactivewnd) data member of the `CWinThread` object.  
