...
```  
 ...
 A standard function signature has been defined for unpacking the parameters of each Windows message and providing type safety. These signatures may be found in the file Afxwin.h in the declaration of [CWnd](../vs140/CWnd-Class.md). Each one is marked with the keyword `afx_msg` for easy identification.  
...
```  
 ...
 Command update messages are routed through the same mechanism, but use the `ON_UPDATE_COMMAND_UI` macro instead. Command update handler member functions take a single parameter, a pointer to a [CCmdUI](../vs140/CCmdUI-Class.md) object, and return `void`. The macro has the form  
...
-   In the implementation file  
 ...
 Advanced users can handle a range of commands by using a single command handler: [ON_COMMAND_RANGE](../vs140/ON_COMMAND_RANGE.md) or `ON_COMMAND_RANGE_EX`. See the product documentation for more information about these macros.  
