...
```  
 ...
 If `_DEBUG` and `_CRTDBG_MAP_ALLOC` are both defined, calls to `_fullpath` and `_wfullpath` are replaced by calls to `_fullpath_dbg` and `_wfullpath_dbg` to allow for debugging memory allocations. For more information, see [_fullpath_dbg, _wfullpath_dbg](../Topic/_fullpath_dbg,%20_wfullpath_dbg.md).  
  
 This function invokes the invalid parameter handler, as described in [Parameter Validation](../vs140/Parameter-Validation.md), if `maxlen` is less than or equal to 0. If execution is allowed to continue, this function sets `errno` to `EINVAL` and returns `NULL`.  
...
|`_tfullpath`|`_fullpath`|`_fullpath`|`_wfullpath`|  
 ...
 If the `absPath` buffer is `NULL`, `_fullpath` calls [malloc](../vs140/malloc.md) to allocate a buffer and ignores the `maxLength` argument. It is the caller's responsibility to deallocate this buffer (using [free](../vs140/free.md)) as appropriate. If the `relPath` argument specifies a disk drive, the current directory of this drive is combined with the path.  
