...
## What can you do with a Domain-Specific Language?  
 ...
 When you have designed a DSL, you can distribute it as part of a Visual Studio Integration Extension (VSIX) package. Users work with the DSL in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)]:  
 ...
 The rest of this overview is a walkthrough that introduces the basic operations of creating and using a domain-specific language in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
...
## Creating a DSL Solution  
 To create a new domain-specific language, you create a new [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] solution by using the Domain-Specific Language project template.  
...
3.  Click **Domain-Specific Language Designer**.  
     ...
     ![Create DSL dialog](../vs140/media/Create_DSLDialog.png "Create_DSLDialog")  
...
-   **Dsl\DslDefinition.dsl** This is the file that you see when you create a DSL solution. Almost all the code in the solution is generated from this file, and most of the changes that you make to a DSL definition are made here. For more information, see Working with the [DSL Definition Diagram](../vs140/Working-with-the-DSL-Definition-Diagram.md).  
  
-   **Dsl project** This project contains code that defines the domain-specific language.  
  
-   **DslPackage project** This project contains code that allows instances of the DSL to be opened and edited in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
...
2.  Press F5, or on the **Debug** menu, click **Start Debugging**.  
     ...
     The DSL builds and is installed in the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
  
     An experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] starts. The experimental instance takes its settings from a separate subtree of the registry, where [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] extensions are registered for debugging purposes. Normal instances of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] do not have access to extensions registered there.  
  
3.  In the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], open the model file named **Test** from **Solution Explorer**.  
...
5.  Click the labels of the shapes to change them.  
 ...
 Your experimental [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] will resemble the following example:  
...
1.  In the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], save the model file.  
...
>  When you change the DSL definition, the sample text template code will not work, unless you update it.  
 ...
 For more information, see [Using Text Templates](../vs140/Generating-Code-from-a-Domain-Specific-Language.md) and [Programming DSLs](../vs140/Writing-Code-to-Customise-a-Domain-Specific-Language.md).  
  
## Customizing the DSL  
 When you want to modify the DSL definition, close the experimental instance and update the definition in the main [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] instance.  
...
3.  Rebuild the DSL.  
    ...
    3.  Press F5. Wait until the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] appears.  
  
4.  In the Debugging solution in the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], open a test model file. Drag elements onto it from the toolbox. Notice that the tool captions and the type names in DSL Explorer have changed.  
...
2.  Add decorators that will display the properties in the shape.  
    ...
    4.  Select each new decorator, and in the Properties window, set the **Position** field. This determines where the domain property value will be displayed on the shape. For example, set **InnerBottomLeft** and **InnerBottomRight**.  
         ...
         ![Compartment shape definition](../vs140/media/FamilyT_Compartment.png "FamilyT_Compartment")  
...
2.  Add a reference relationship between Person and Town.  
    ...
    1.  Click the **Reference Relationship** tool, click Person and then click Town.  
         ...
         ![DSL definition fragment: family tree root](../vs140/media/FamilyT_Root.png "FamilyT_Root")  
...
5.  Create a connector to display the relationship between Person and Towns.  
    ...
    2.  Use the **Diagram Element Map** tool to link the new connector to the relationship between Person and Town.  
         ...
         ![Family Tree definition with added shape map](../vs140/media/FamilyT_ShapeMap.png "FamilyT_ShapeMap")  
...
9. In the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)], open a test model file. Use the new tools to create towns and links between towns and persons. Notice that you can only create links between the correct types of element.  
...
10. Create code that lists the town in which each person lives. Text templates are one of the places where you can run such code. For example, you could modify the existing Sample.tt file in the Debugging solution so that it contains the following code:  
    ...
    ```  
     ...
     When you save the *.tt file, it will create a subsidiary file that contains the list of people and their residences. For more information, see [Generating Code from a Domain-Specific Language](../vs140/Generating-Code-from-a-Domain-Specific-Language.md).  
  
## Validation and Commands  
 You could develop this DSL further by adding validation constraints. These constraints are methods that you can define, that make sure that the model is in a correct state. For example, you could define a constraint to make sure that the birth date of a child is later than that of its parents. The validation feature displays a warning if the DSL user tries to save a model that breaks any of the constraints. For more information, see [Adding Validation](../Topic/Validation%20in%20a%20Domain-Specific%20Language.md).  
  
 You can also define menu commands that the user can invoke. Commands can modify the model. They can also interact with other models in [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] and with external resources. For more information, see [Adding Custom Commands](../vs140/How-to--Modify-a-Standard-Menu-Command-in-a-Domain-Specific-Language.md).  
  
## Deploying the DSL  
 To allow other users to use the domain-specific language, you distribute a [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] Extension (VSIX) file. This is created when you build the DSL solution.  
  
 Locate the .vsix file in the bin folder of your solution. Copy it to the computer on which you want to install it. On that computer, double-click the VSIX file. The DSL can be used in all instances of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] on that computer.  
  
 You can use the same procedure to install the DSL on your own computer so that you do not have to use the experimental instance of [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)].  
  
 For more information, see [Deploying a DSL](../vs140/Deploying-Domain-Specific-Language-Solutions.md).  
  
##  <a name="Reset"></a> Removing old Experimental DSLs  
 If you have created experimental DSLs that you no longer want, you can remove them from your computer by resetting the [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] Experimental instance.  
  
 This will remove from your computer all experimental DSLs and other experimental [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] extensions. These are extensions that have been executed in debugging mode.  
  
 This procedure does not remove DSLs or other [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] extensions that have been fully installed by executing the VSIX file.  
...
2.  Rebuild any experimental DSLs or other experimental [!INCLUDE[vsprvs](../vs140/includes/vsprvs_md.md)] extensions that you still want to use.  
  
## See Also  
 [Models, classes and relationships](../vs140/Understanding-Models--Classes-and-Relationships.md)   
