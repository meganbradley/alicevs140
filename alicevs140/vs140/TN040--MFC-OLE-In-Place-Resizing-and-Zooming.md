...
## MFC Support for Zooming  
 ...
 For an example of zooming correctly see the MFC OLE sample [HIERSVR](../vs140/Visual-C---Samples.md). Zooming in HIERSVR is complicated by the fact that it displays text, and text, in general, does not scale in a linear fashion (hints, typographic conventions, design widths, and heights all complicate the matter). Still, HIERSVR is a reasonable reference for implementing zooming correctly, and so is the MFC Tutorial [SCRIBBLE](../vs140/Visual-C---Samples.md) (step 7).  
...
## MFC Support for In-Place Resizing  
 ...
 You can implement custom resizing and still leverage the user interface provided by `COleResizeBar` by overriding the **WM_SIZECHILD** message in your `COleIPFrameWnd` class. For more information on the specifics of **WM_SIZECHILD**, see [Technical Note 24](../vs140/TN024--MFC-Defined-Messages-and-Resources.md).  
