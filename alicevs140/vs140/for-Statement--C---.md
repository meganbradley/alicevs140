...
```  
 ...
 A `for` loop terminates when a [break](../vs140/break-Statement--C---.md), [return](../vs140/return-Statement--C---.md), or [goto](../vs140/goto-Statement--C---.md) (to a labeled statement outside the **for** loop) within `statement` is executed. A [continue](../vs140/continue-Statement--C---.md) statement in a `for` loop terminates only the current iteration.  
...
```  
 ...
 By default, under [/Ze](../Topic/-Za,%20-Ze%20\(Disable%20Language%20Extensions\).md), a variable declared in a `for` loop remains in scope until the `for` loop's enclosing scope ends.  
  
 [/Zc:forScope](../Topic/-Zc:forScope%20\(Force%20Conformance%20in%20for%20Loop%20Scope\).md) enables standard behavior of variables declared in for loops without needing to specify /Za.  
